...
class Node:
    ...
    def get_next_bits(self, block_id, log=False):
        ...
        #Caluclate how long this difficulty period lasted
...
def mine_forever(public_key):
    ...
    while True:
        ...
        if mined_block:
            logger.info("")
            logger.info(f"Mined a block w/ a subsidy of {mined_block.txns[0].tx_outs[0].amount}")
            with lock:
                node.handle_block(mined_block)

def mine_genesis_block(node, public_key):
    coinbase = prepare_coinbase(public_key, node.get_block_subsidy(), tx_id="abc123")
    unmined_block = Block(txns=[coinbase], prev_id=None, nonce=0, bits=INITIAL_DIFFICULTY_BITS, timestamp=1574913459.674626)
    mined_block = mine_block(unmined_block)
